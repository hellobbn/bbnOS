//===- printf.c - basic implementation of the printf function -------------===//
//
...
///        up eventually
///
#define putchar(c) (fb_putchar(c, FB_CLOR_WHITE, FB_CLOR_BLACK))
...
///         this function
static char *__int_str(long i, char b[], int base, bool plusSignIfNeeded,
  ...
  for (int i = 0; format[i]; ++i) {
    ...
    if (format[i] == '%') {
      ...
      case 'u': {
        ...
        case 'j': {
          ulong integer = va_arg(list, ulong);
          __int_str(integer, intStrBuffer, base, plusSign, spaceNoSign,
                    lengthSpec, leftJustify, zeroPad);
          displayString(intStrBuffer, &chars);
          break;
